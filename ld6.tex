...
\end{equation}
where $(\alpha\le u) \de \alpha$ is the ``$\alpha$--layer'' of $u$ and $(\alpha\le u)$ is the indicator function of the event in brackets.   To prove \eref{decompose} note the left hand side $u=\int_0^u 1\de \alpha$ which equals the right hand side.
...
\section{Layer dependence and measure of tail behaviour}\label{sliterature}
